...
function ChatComponent({ receiverId, receiverUsername }: ChatComponentProps) {
    ...
    useEffect(() => {
        ...
        connection.on('ReceiveMessage', (messageDto: ChatMessageDto) => {
            ...
            if (isRelevant) {
                setMessages(prev =>
                    prev.some(m => m.id === messageDto.id) ? prev : [...prev, messageDto]
                );
            }
        });
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                const res = await fetch(`https://localhost:7039/api/chat/messages/${receiverId}`, {
...
>
            ...
            <div style={{ flexGrow: 1, overflowY: 'auto', border: '1px solid #eee', padding: 10, marginBottom: 10 }}>
                {receiverId === null ? (
                    ...
                    messages
                        ...
                        .map(m => (
                            <div key={m.id} style={{
                                ...
                                padding: 10, marginBottom: 5, borderRadius: 5
                            }}>
                                ...
                                <div style={{ fontSize: '0.8em', color: '#666' }}>
                                    ...
                                    {m.senderId === user?.id && (
                                        <span style={{ marginLeft: 10 }}>
                                            <button onClick={() => handleEditMessage(m.id, m.message)} style={{ marginRight: 5 }}>Edit</button>
