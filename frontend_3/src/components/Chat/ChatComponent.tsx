...
function ChatComponent({ receiverId, receiverUsername }: ChatComponentProps) {
  ...
  useEffect(() => {
    if (!isAuthenticated || !receiverId) {
      if (!isAuthenticated)
        console.warn("User not authenticated, not connecting to SignalR.");
      if (!receiverId)
        console.warn("No receiver selected, not connecting to SignalR.");
      ...
      if (connection) {
        connection
          ...
          .then(() =>
            console.log(
              "SignalR connection stopped due to no receiver or unauthentication."
            )
          )
    ...
    return () => {
      if (newConnection) {
        newConnection
          ...
          .catch((e) =>
            console.error("Error stopping connection during cleanup:", e)
          );
      }
    };
    ...
    if (connection) {
      ...
      connection.on("MessageEdited", (editedMessageDto: ChatMessageDto) => {
        ...
        setMessages((prevMessages) =>
          prevMessages.map((msg) =>
            msg.Id === editedMessageDto.Id ? editedMessageDto : msg
          )
        );
      });
      ...
      connection
        ...
        .catch((e) => {
          ...
          if (e.statusCode === 401) {
            console.error(
              "Authentication failed for SignalR. Please log in again."
            );
          }
        });
  ...
  const handleSendMessage = async () => {
    ...
    if (!user) {
      console.warn(
        "Cannot send message: Current user information not available."
      );
      return;
    }
    if (!receiverId) {
    ...
    >
        ...
        {receiverId === null ? (
          ...
          messages.map((msg: ChatMessageDto, index) => (
            <div
              ...
              style={{
                marginBottom: "5px",
                background: msg.SenderId === user?.id ? "#e0f7fa" : "#fce4ec",
                padding: "5px",
                borderRadius: "5px",
              }}
            >
              <strong>{msg.SenderUsername || msg.SenderId}:</strong>{" "}
              {msg.Message}
              <span
      ...
      <div>
        ...
        <button
          ...
          disabled={!connection || !isAuthenticated || !user || !receiverId}
        >
          Send
        </button>
      </div>
      ...
      {!isAuthenticated && (
        <p style={{ color: "red" }}>Please log in to chat.</p>
      )}
      {!connection && isAuthenticated && (
        <p style={{ color: "orange" }}>Initializing chat...</p>
      )}
      {connection && connection.state !== "Connected" && isAuthenticated && (
        <p style={{ color: "orange" }}>Connection status: {connection.state}</p>
      )}
      {connection && connection.state === "Connected" && isAuthenticated && (
        <p style={{ color: "green" }}>Connected to chat!</p>
      )}
    </div>
  );
}
