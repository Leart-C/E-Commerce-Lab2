...
const Payment: React.FC = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await axiosInstance.get(paymentApiUrl);
  ...
  const fetchPaymentMethods = async () => {
    try {
      const response = await axiosInstance.get(paymentMethodApiUrl);
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "paymentMethodId" || name === "orderId"
          ? Number(value)
          : value,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId !== null) {
        await axiosInstance.put(`${paymentApiUrl}/${editingId}`, formData);
      } else {
        await axiosInstance.post(paymentApiUrl, formData);
      }
      setFormData({});
      setEditingId(null);
      setOpenModal(false);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("A jeni i sigurt që dëshironi të fshini këtë pagesë?")) {
      try {
        await axiosInstance.delete(`${paymentApiUrl}/${id}`);
        fetchPayments();
      } catch (error) {
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>
                <strong>ID Porosisë</strong>
              </TableCell>
              <TableCell>
                <strong>Metoda Pagesës</strong>
              </TableCell>
              <TableCell>
        ...
        <DialogContent>
          <form onSubmit={handleSubmit}>
            ...
            <FormControl fullWidth margin="normal" required>
              <InputLabel id="payment-method-label">Metoda Pagesës</InputLabel>
              <Select
              ...
              >
                <MenuItem value="">
                  <em>Zgjidh Metodën e Pagesës</em>
                </MenuItem>
